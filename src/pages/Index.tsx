...
export default function App() {
  ...
  Prism.hooks.add("after-highlight", (env) => {
    env.highlightedCode = env.highlightedCode.replace(
      regex,
      (match) =>
        `<span class="inline-flex w-3 h-3 rounded ring-1 ring-gray-900/30 mr-1"></span>${match}`
    );
  ...
  return (
    <div class="fade_in relative w-full min-h-screen flex flex-col md:flex-row px-2 pb-24 md:px-0 md:pt-2 md:pr-2 md:pb-2 bg-base-100">
      ...
      <main
        data-outlet
        class="w-full max-w-[100dvw] overflow-hidden min-h-[calc(100dvh-3rem)] transition-all duration-300 ease-in-out rounded-xl md:rounded-md p-2 md:p-4 shadow-sm ring-1 bg-base-200 ring-base-100/5"
      />
  ...
  return (
    <Menu vertical>
      {documentationLinks.map((link, index) => {
          ...
          return (
            <SubMenu title={link.title} basePath="/components">
              {link.children.map((child) => (
                <MenuItem onClick={() => closeModal("mobile-menu")}>
        ...
        return (
          <MenuItem
            class={clsx(index === 0 && "mt-4")}
            onClick={() => closeModal("mobile-menu")}
          >
...
function MobileMenu() {
  return (
    <div class="md:hidden w-full flex items-center justify-between py-4 px-4">
