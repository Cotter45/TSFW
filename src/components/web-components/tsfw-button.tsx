class MyButton extends HTMLElement {
  ...
  static get observedAttributes() {
    return ["type", "class", "styleType", "color"];
  }
  ...
  renderButton() {
    ...
    const baseClasses = styles.base.join(" ");
    const styleType = this.getAttribute("styleType") as
      | keyof typeof styles
      | "solid";
    const colorType = this.getAttribute("color") as
      | keyof typeof styles.colors
      | "light";

    const typeClasses = Array.isArray(styles[styleType])
      ? (styles[styleType] as string[]).join(" ")
      : "";
    const colorClasses = Array.isArray(styles.colors[colorType])
      ? (styles.colors[colorType] as string[]).join(" ")
      : "";

    this.button.className = `${baseClasses} ${typeClasses} ${colorClasses} ${
      this.getAttribute("class") || ""
    }`;

    this.button.type =
      (this.getAttribute("type") as "button" | "submit" | "reset") || "button";

    if (this.hasAttribute("onclick")) {
      const onclickHandler = new Function(this.getAttribute("onclick") || "");
      this.button.onclick = onclickHandler.bind(this);
    }
  }
}

customElements.define("tsfw-button", MyButton);
