...
const userState = createPersistentState<UserState>("users-idb", "idb", [
...
export function IDBState() {
  // Subscribe to user state changes to update the UI
  userState.subscribe((state) => {
    console.log("Indexed DB user state changed", state);

    // Update age text directly by ID if needed
    state.forEach((user, index) => {
      const ageElement = document.getElementById(`idb-age-text-${index}`);
      if (ageElement) {
      ...
      <tbody>
        {userState.getState().map((user, index) => (
          <tr key={user.id}>
            ...
            <td class="py-4 px-6 border-b border-zinc-300 dark:border-zinc-600">
              <Text id={`idb-age-text-${index}`}>{user.age}</Text>
            </td>
